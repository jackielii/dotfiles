...
require("lazy").setup({
...

  {
    ...
    keys = {
      ...
      { "<leader>ci",        [[<cmd>FzfLua live_grep<cr>]], desc = "FzfLua live grep" },
  ...
  { "echasnovski/mini.bufremove", lazy = true },
  {
    ...
    opts = function()
      ...
      vim.api.nvim_set_hl(0, "MyBufferSelected",
        ...
        sections = {
            ...
            },
            {
              function(self)
                ...
                if vim.bo.modified then
                  parts[#parts] = Util.lualine.format(self,
                    parts[#parts], "Constant")
                end
          ...
          lualine_x = {
            ...
            },
            {
              ...
              cond = function()
                return package.loaded["molten"] and
...

  {
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd("User", {
        ...
        command =
        [[imap <buffer><expr> <CR> coc#pum#visible() ? coc#pum#confirm() : "\<Plug>(VM-I-Return)"]],
      })
    end,
  },
          ...
          require("fzf-lua").files({
            ...
            actions = {
              ["ctrl-x"] = function()
                ...
                vim.fn["wiki#page#open"](require("fzf-lua")
                  .get_last_query())
              end,
            },
          })
  ...
  --     }
  --   end,
  --   config = function(_, plugOpts)
  --     require("telescope").setup(plugOpts)
  --
  --     local grep_layout_config = {
  --       layout_strategy = "bottom_pane",
  --       -- layout_config = { bottom_pane = { prompt_position = "bottom" } },
  --       -- sorting_strategy = "descending",
  --     }
  --     map("n", "<leader>dgg",
  --       function() require("telescope.builtin").grep_string(grep_layout_config) end,
  ...
  --       local word = vim.fn.substitute(vim.fn.getreg("@"), "\n$", "", "g")
  --       vim.fn.setreg("@", save)
  --       require("telescope.builtin").grep_string(vim.tbl_extend("force", grep_layout_config,
  --         { search = word }))
  --     end
  --     map("n", "<leader>dg",
  ...
  --
  --     local search_string = function(opts)
  --       opts = opts or {}
  --       local cwd = opts.cwd or vim.fn.getcwd()
  --       local text = vim.fn.input({ prompt = "search " .. cwd .. ": " })
  --       if text == "" then return end
  --       require("telescope.builtin").grep_string(vim.tbl_extend("force", grep_layout_config, opts,
  --         { search = text }))
  --     end
  --
  --     map("n", "<leader>/", function() search_string() end,
  --       { desc = "Telescope grep literal string (cwd)" })
  --     map("n", "<leader>?", function() search_string({ cwd = vim.fn.expand("%:p:h") }) end,
  --       { desc = "Telescope grep literal string (%:p:h)" })
  --
  --
  --     local rg_param_args = {
  --       "-t", "--type",
  ...
  --       "-e", "--regexp",
  --     }
  --     -- table.move(rg_args, 1, #rg_args, #rg_param_args + 1, rg_args)
  --     -- table.move(rg_param_args, 1, #rg_param_args, 1, rg_args)
  --     -- vim.list_extend(rg_args, rg_param_args) -- this appends the list to the end of the first list
  --     for i, v in ipairs(rg_param_args) do table.insert(rg_args, i, v) end
  ...
  --         grep_layout_config, { additional_args = opts, use_regex = true },
  --         { search = table.concat(search, " ") }))
  --     end, {
  --       nargs = "+",
  --       complete = function(_, line)
  ...
  --         end, rg_args)
  --       end,
  --       desc = "CocGrep using telescope"
  --     })
  --     map("n", "<leader>cg", ":TelescopeGrep<space>")
  --   end,
  --   dependencies = {
...

  {
    ...
    keys = {
      ...
      {
        "<leader>f",
        [[<cmd>FloatermNew lf -command 'map l open' -command 'map o ${{open $f}}' %<cr>]],
        desc = "Lf",
      },
    },
    init = function()
        ...
        callback = function()
          ...
          for k, v in pairs(maps) do
            map(
              { "t" },
              k,
              '<cmd>let b:floaterm_opener = "' ..
              v .. '"<cr><cmd>call feedkeys("l", "i")<cr>',
              { buffer = true }
            )
          end
        end,
